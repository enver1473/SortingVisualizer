...
public class TimSort extends Sort {
	private TimSort timSortInstance;
	 ...
	 *
	 * Unlike Tim's original C version, we do not allocate this much storage when
	 ...
	 */
	public TimSort() {
		this.setLabels("Tim Sort", "Tim");
	}

	/*
	 * The next two methods (which are static, one being package private) constitute
	 * the entire API of this class. Each of these methods obeys the contract of the
	 * public method with the same signature in java.util.Arrays.
	 */
	public static void sort(TimSort timSort, int[] a, int length) {
		sort(timSort, a, 0, length);
	}

	static void sort(TimSort timSort, int[] a, int lo, int hi) {
		TimSort ts = timSort;
	 ...
	 * @param start the index of the first element in the range that is not already
	 *              known to be sorted (@code lo <= start <= hi}
	 * @param c     comparator to used for the sort
	 */

	// Here, we do not use the Binary Insertion Sort included in ArrayVisualizer, as
	// TimSort
	// outfits it with a start index and uses the arraycopy method

	@SuppressWarnings("fallthrough")
	private static void binarySort(TimSort ts, int[] a, int lo, int hi, int start) {
		if (start == lo)
			start++;
		...
		for (; start < hi; start++) {
			 ...
			 * equal to pivot, left points to the first slot after them -- that's why this
			 * sort is stable. Slide elements over to make room for pivot.
			 */
			int n = start - left; // The number of elements to move
			// Switch is just an optimization for arraycopy in default case
			switch (n) {
			case 2:
				ts.writes.write(a, left + 2, a[left + 1], 1, true, false);
			case 1:
				ts.writes.write(a, left + 1, a[left], 1, true, false);
				break;
			default:
				ts.writes.reversearraycopy(a, left, a, left + 1, n, 1, true, false);
			}
			ts.writes.write(a, left, pivot, 1, true, false);
			ts.highlights.clearAllMarks();
		}
	}
	 ...
	 * run will always be ascending when the method returns).
	 *
	 ...
	 * a[lo] > a[lo + 1] > a[lo + 2] > ...
	 *
	 * For its intended use in a stable mergesort, the strictness of the definition
	 * of "descending" is needed so that the call can safely reverse a descending
	 ...
	 *         specified array
	 */
	private static int countRunAndMakeAscending(TimSort ts, int[] a, int lo, int hi) {
		int runHi = lo + 1;
		if (runHi == hi)
			return 1;
	 ...
	 */
	private static void reverseRange(TimSort ts, int[] a, int lo, int hi) {
	 ...
	 *
	 * If n < MIN_MERGE, return n (it's too small to bother with fancy stuff). Else
	 * if n is an exact power of 2, return MIN_MERGE/2. Else return an int k,
	 ...
	 * 
	 * Thanks to Stijn de Gouw, Jurriaan Rot, Frank S. de Boer, Richard Bubel and
	 * Reiner Hahnle, this is fixed with respect to the analysis in "On the
	 * Worst-Case Complexity of TimSort" by Nicolas Auger, Vincent Jug, Cyril
	 * Nicaud, and Carine Pivoteau.
	 */
	 ...
	 * @param hint the index at which to begin the search, 0 <= hint < n. The closer
	 *             hint is to the result, the faster this method will run.
	 * @param c    the comparator used to order the range, and to search
	 * @return the int k, 0 <= k <= n such that a[b + k - 1] < key <= a[b + k],
	 *         pretending that a[b - 1] is minus infinity and a[b + n] is infinity.
	 *         In other words, key belongs at index b + k; or in other words, the
	 *         first k elements of a should precede key, and the last n - k should
	 *         follow it.
	 */
	private static int gallopLeft(TimSort ts, int key, int[] a, int base, int len, int hint) {
		int lastOfs = 0;
		int ofs = 1;
	 ...
	 * @return the int k, 0 <= k <= n such that a[b + k - 1] <= key < a[b + k]
	 */
	private static int gallopRight(TimSort ts, int key, int[] a, int base, int len, int hint) {
		int ofs = 1;
		int lastOfs = 0;
	 ...
	 * twin, mergeHi should be called if len1 >= len2. (Either method may be called
	 * if len1 == len2.)
	 *
	 * @param base1 index of first element in first run to be merged
	 * @param len1  length of first run to be merged (must be > 0)
	 * @param base2 index of first element in second run to be merged (must be aBase
	 *              + aLen)
	 * @param len2  length of second run to be merged (must be > 0)
	 */
	private void mergeLo(TimSort ts, int base1, int len1, int base2, int len2) {
		...
		outer: while (true) {
			 ...
			 */
			do {
				if (ts.reads.compare(a[cursor2], tmp[cursor1]) < 0) {
					ts.writes.write(a, dest++, a[cursor2++], 1, false, false);
					ts.markArray(1, dest);
					ts.markArray(2, cursor2);
					count2++;
	 ...
	 * @param len2  length of second run to be merged (must be > 0)
	 */
	private void mergeHi(TimSort ts, int base1, int len1, int base2, int len2) {
		...
		outer: while (true) {
			 ...
			 */
			do {
				if (ts.reads.compare(tmp[cursor2], a[cursor1]) < 0) {
					ts.writes.write(a, dest--, a[cursor1--], 1, false, false);
					ts.markArray(1, dest);
					ts.markArray(2, cursor1);
					count1++;
	...
	public void init(Main mainFrame, CanvasPane canvas, Reads reads, Writes writes, Delays delays,
		...
		this.a = this.canvas.getArray();
		this.len = this.canvas.count;

		// Allocate temp storage (which may be increased later if necessary)
		int[] newArray = new int[this.len < 2 * INITIAL_TMP_STORAGE_LENGTH ? this.len >>> 1
		 ...
		 * same stack length (85), but this was measured to be too expensive when
		 * sorting "mid-sized" arrays (e.g., 100 elements) in Java. Therefore, we use
		 * smaller (but sufficiently large) stack lengths for smaller arrays. The
		 * "magic numbers" in the computation below must be changed if MIN_MERGE is
		 * decreased. See the MIN_MERGE declaration above for more information.
		 */
		int stackLen = (this.len < 120 ? 5 : this.len < 1542 ? 10 : this.len < 119151 ? 19 : 40);
	...
	public void run(Main mainFrame, CanvasPane canvas, Reads reads, Writes writes, Delays delays,
			Highlights highlights) {
		init(mainFrame, canvas, reads, writes, delays, highlights);

		this.canvas.getShuffles().shuffle();

		this.timSortInstance = new TimSort();
		this.timSortInstance.init(mainFrame, canvas, reads, writes, delays, highlights);

		TimSort.sort(this.timSortInstance, this.canvas.getArray(), this.canvas.count);
